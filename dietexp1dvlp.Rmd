...
total_larvaltime <- ggplot(ddata.tidy %>% filter(stage == "HatchDate" | stage=="PupDate"), aes(x=date,y=stage,group=Diet,color=factor(Diet), fill=factor(Diet))) +
...
totaldata1new <- na.omit(totaldata1)
...
d2plot <-ggplot(totaldata1, aes(x=numDaysHatchToEclose, y=hind.area)) +
  geom_smooth(method="lm") 
print(d2plot)

d3 <- lm(Melarea~numDaysHatchToEclose, data=totaldata1)
d3aov <- aov(d3, type="marginal", data=totaldata1)
anova(d3aov)

d3plot <- ggplot(totaldata1, aes(x=numDaysHatchToEclose, y=Melarea)) +
  geom_smooth(method="lm")
print(d3plot)
