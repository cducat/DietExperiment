...
df<-data.frame(LarvID=larvae_ids,overall_DR=overall_DR,hatchToIns5_DR=hatchToIns5_DR,numDaysHatchToIns5=numDaysHatchToIns5,critical_period=critical_period, numDaysHatchtoPup=numDaysHatchtoPup)
...
ggplot(data=ddata.tidy,aes(x=date,y=stage,group=Diet,color=factor(Diet),fill=factor(Diet)))+
  geom_smooth(aes(y=stage),se=TRUE) 

ggplot(data=ddata.tidy %>% filter(stage == "Ins5" | stage== "PupDate"),aes(x=date,y=stage,group=Diet,color=factor(Diet),fill=factor(Diet)))+
  geom_smooth(aes(y=stage),method="lm",se=TRUE) 

summary(ddata.tidy$stage)
#HELP
compare_means(fore.area ~ diet, data=dietmorph2)
my_comparisons <- list(c("N", "T"))

ggplot(data=ddata %>% group_by(Diet) %>% summarize(avDR=mean(overall_DR))) +
  geom_segment()
