...
jointdata <- merge(ddata, dietmorph2, by="specimen")

names(dietcolor1)[names(dietcolor1)=="Specimen"] <- "specimen"
totaldata <- merge(jointdata, dietcolor1, by="specimen")
```
...
d1 <- gls(fore.area~overall_DR.x, data=totaldata)
d1aov <- aov(d1, type="marginal", data=totaldata)
anova(d1aov)

d1plot <- ggplot(totaldata, aes(x=overall_DR.x, y=fore.area)) +
